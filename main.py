...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are SAM, Tony Stark's AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "SAM checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Sam. "
...
class SamLive:
    ...
    def __init__(self, ui: SamUI):
        self.ui             = ui
        self._asst_name     = "SAM"   # updated each session from config
        self.session              = None
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask SAM to
    ...
    def interrupt(self) -> None:
        """Stop SAM mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[SAM] ✋ Interrupted — {drained} audio chunks discarded")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "SAM").strip()
        ...
        _addr = (f"ADDRESS: Always call the user 'BOSS'. The user's real name is '{_user_name}'. Only use the real name when explicitly asked 'who am I' or 'what is my name'."
                 if _user_name
        ...
        cfg = dict(
            ...
            # window — SAM can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Affective dialog: SAM hears tone/emotion and adapts its voice.
            # Proactive audio: SAM stays silent when speech isn't addressed
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[SAM] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        print("[SAM] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                sam_speaking = self._is_speaking
            if not sam_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until SAM finishes speaking the answer
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        name_clause = f" Address the user as BOSS."
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session:
                # Don't interrupt if user spoke recently or SAM is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[SAM] Connected.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: SAM online.")
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[SAM] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[SAM] Reconnecting in {delay}s...")
