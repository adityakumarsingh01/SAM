...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "S.A.M", parent=None):
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "sam"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("sam:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for SAM", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure S.A.M. before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="SAM", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "SAM"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Sam actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — SAM ready")
...
class HoverTitleBar(QWidget):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(f"background: {C.BG}; border-bottom: 1px solid {C.PRI_DIM};")
        ...
        btn_style = f"""
            QPushButton {{
                ...
                border: none; font-size: 14px; padding: 5px 10px;
            }}
            QPushButton:hover {{ color: {C.PRI}; background: rgba(255, 255, 255, 0.1); }}
        """
        close_style = f"""
            QPushButton {{
                color: {C.TEXT_DIM}; background: transparent;
        ...
        self.rest_btn.setStyleSheet(btn_style)
        self.rest_btn.clicked.connect(lambda: self.window().showNormal())
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "SAM").strip()
        ...
        self.setWindowTitle(f"{_display} — SAM")
        ...
        self.on_interrupt      = None   # callable: () -> None — stop SAM mid-speech
        self.get_plugins       = None   # callable: () -> list[dict], set by SamLive
        self._muted            = False
        self._current_file: str | None = None
        ...
        self._title_bar = HoverTitleBar(self)
        self._title_bar.setGeometry(0, -30, self.width(), 30)
        self._title_anim = QPropertyAnimation(self._title_bar, b"pos")
        self._title_anim.setDuration(200)
        self._title_anim.setEasingCurve(QEasingCurve.Type.OutQuad)
        
        self._hover_timer = QTimer(self)
        self._hover_timer.timeout.connect(self._check_hover)
        self._hover_timer.start(100)
        self._bar_is_down = False
    ...
    def _check_hover(self):
        ...
        try:
            ...
            in_zone = (cursor_pos.y() < 10 or (self._bar_is_down and cursor_pos.y() < 40))
            
            if in_zone and not self._bar_is_down:
                self._bar_is_down = True
                self._title_anim.setStartValue(QPoint(0, -30))
                self._title_anim.setEndValue(QPoint(0, 0))
                self._title_anim.start()
            elif not in_zone and self._bar_is_down:
                self._bar_is_down = False
                self._title_anim.setStartValue(QPoint(0, 0))
                self._title_anim.setEndValue(QPoint(0, -30))
                self._title_anim.start()
        except Exception:
            pass
    ...
    def resizeEvent(self, event):
        ...
        try:
            if hasattr(self, '_title_bar'):
                self._title_bar.resize(self.width(), 30)
                if not self._bar_is_down:
                    self._title_bar.move(0, -30)
                else:
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_sam_icon(out_path: Path) -> bool:
        """
        Render a SAM arc-reactor icon at 4× resolution and downsample
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        try:
            ...
            sc.Description      = "S.A.M AI Assistant"
    ...
    def _create_desktop_shortcut(self):
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "sam.ico"
        if not ico_path.exists():
            self._build_sam_icon(ico_path)
        ...
        try:
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>SAM</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.sam.assistant</string>\n'
                    '  <key>CFBundleName</key><string>S.A.M</string>\n'
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("SAM", C.PRI_DIM))
        lay.addSpacing(8)
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "SAM_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.sam.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "sam.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "SAM_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "SAM_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.sam.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.sam.assistant</string>\n'
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "SAM") or "SAM",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self._assistant_name = name.strip() or "SAM"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — SAM")
        self._title_lbl.setText(display)
        if display in ("SAM", "S.A.M"):
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "SAM") or "SAM"
